...
class Script:
	...
	def main():
		...
		pantry_counts = haxe_ds_StringMap()
		_g = 0
		while (_g < len(pantry)):
			item = (pantry[_g] if _g >= 0 and _g < len(pantry) else None)
			_g = (_g + 1)
			if item in pantry_counts.h:
				value = (pantry_counts.h.get(item,None) + 1)
				pantry_counts.h[item] = value
			else:
				pantry_counts.h[item] = 1
		print(str(pantry_counts.toString()))
