...
class Script:
	...
	_hx_statics = ["main", "tokenize", "clean_text", "get_tokenized_vocabulary", "find_in_array"]

	@staticmethod
	def main():
		tokenized_vocabulary = Script.get_tokenized_vocabulary()
		story_string = sys_io_File.getContent("story.txt")
		clean_chars = [",", ".", "'", ";", "\n"]
		tokenized_story = Script.tokenize(story_string,clean_chars)
		misspelled_words = list()
		_g = 0
		while (_g < len(tokenized_story)):
			word = (tokenized_story[_g] if _g >= 0 and _g < len(tokenized_story) else None)
			_g = (_g + 1)
			is_not_in_vocabulary = (not Script.find_in_array(word,tokenized_vocabulary))
	...
	def tokenize(text_string,special_characters):
		cleaned_story = Script.clean_text(text_string,special_characters)
	...
	def clean_text(text_string,special_characters):
		_g = 0
		while (_g < len(special_characters)):
			special_character = (special_characters[_g] if _g >= 0 and _g < len(special_characters) else None)
			_g = (_g + 1)
