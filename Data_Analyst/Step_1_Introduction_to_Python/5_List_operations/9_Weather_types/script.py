...
class Script:
	...
	_hx_statics = ["main", "getWeather", "getWeatherData", "findInArray"]

	@staticmethod
	def main():
		weather = Script.getWeather()
		new_weather = weather[1:len(weather)]
		weather_types = ["Rain", "Sunny", "Fog", "Fog-Rain", "Thunderstorm", "Type of Weather"]
		weather_type_found = list()
		_g = 0
		while (_g < len(weather_types)):
			weather_type = (weather_types[_g] if _g >= 0 and _g < len(weather_types) else None)
			_g = (_g + 1)
			x = Script.findInArray(weather_type,new_weather)
			weather_type_found.append(x)
		print(str(weather_type_found))
