...
class Script {
	...
	static function main() {
		...
		var weather_types = ["Rain", "Sunny", "Fog", "Fog-Rain", "Thunderstorm", "Type of Weather"];
		
		var weather_type_found = new Array<Bool>();
		
		for ( weather_type in weather_types ) {
			weather_type_found.push( findInArray( weather_type, new_weather ));
		}
		
		trace( weather_type_found );
	}
