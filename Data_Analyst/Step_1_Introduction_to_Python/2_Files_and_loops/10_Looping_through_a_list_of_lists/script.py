...
class Script:
	...
	_hx_statics = ["main", "get_final_data"]

	@staticmethod
	def main():
		final_data = Script.get_final_data()
		cities_list = []
		_g = 0
		while (_g < len(final_data)):
			row = (final_data[_g] if _g >= 0 and _g < len(final_data) else None)
			_g = (_g + 1)
			cities_list.append((row[0] if 0 < len(row) else None))
